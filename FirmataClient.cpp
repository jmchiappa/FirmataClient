...
// processing incoming data for each peripheral
void sysex_callback_extended(byte command, byte argc, byte *argv) __attribute__((weak));

// sending data to the client from the periodic loop
void execute_extended(void) __attribute__((weak));

// return true when specific IO used by one more peripherals try to be used
bool is_pin_protected(byte pin) __attribute__((weak));

// callback from CAPABILITY QUERY, return a 16 bits-map (in fact 14 bits)
uint16_t getRegisteredPeripheral(void) __attribute__((weak));
...
// wrapper if no optional peripheral is defined
...
// hook for execute call
void execute_extended(void) {}

// hook for specific protected pins
bool is_pin_protected(byte pin) {
	return false;
}

// hook for optional peripheral reset callback
void reset_callback_extended(void){};
