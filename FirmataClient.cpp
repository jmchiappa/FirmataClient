...
#define IS_PIN_PROTECTED(p) ((p==38)||(p==39)||(p==33)||(p==34))   // PC6=D38, see variant.h
#else
#define IS_PIN_PROTECTED(p) (false)
#endif
...
// if a proximity sensor has been defined, this command is allowed. otherwhise, not
    case 0x01: // Special command id, VL6180X I2C access or VL53X0 Time of Flight
    if((argv[1]&0x01)==0x01)
    {
...
#ifdef STM_STEPPER
    case 0x02: // special command for step motor DRV8825
...
#ifdef WS2812_SCREEN
    case 0x03 : // special ID for ws2812 strip LED as 8x8 matrix
...
//      argc=decodeByteStream(argc,argv); // decode le stream
      ...
      switch(argv[0])     // traite la commande demandée
      {
        ...
        case 0x06 : // Bitmap
          ...
          //     BitmapBuffer
          //   {
          //     WS2812Screen.SetPixelAt(argv[1],(int8_t)argv[2]+x,(int8_t)argv[3]+y,
          //     argv[idx]<<1,argv[idx+1],argv[idx+2],argv[idx+3]);
          //     idx+=4;
          //   }


          // for(uint16_t idx=0;idx<argv[4]*argv[5]*4;idx++)
          //   BitmapBuffer[idx] = argv[6+idx]; // Reconstruit le buffer d'image


          // for(uint16_t idx=0;min(argv[4]*argv[5]*4, sizeof(BitmapBuffer));idx++)
          //   BitmapBuffer[idx] = (&(argv[6]))[idx]<<1; // Reconstruit le buffer d'image

          // WS2812Screen.Bitmap(argv[1],(int8_t)argv[2],(int8_t)argv[3],argv[4],argv[5],BitmapBuffer);


          // for( uint16_t i=0,j=0;i++;i<argv[4]*argv[5])
          // {
          //   BitmapBuffer[j++]=argv[7+i]; // copy all data
          // }
...
void systemResetCallback()
{
  ...
  for (byte i = 0; i < TOTAL_PINS; i++) {
    ...
    } else if (IS_PIN_DIGITAL(i) && (!IS_PIN_SERIAL(i)) && (!IS_PIN_PROTECTED(i))) {
