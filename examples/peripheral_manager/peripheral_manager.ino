...
HardwareSerial SerialFirmata(D0,D1);  // USART 2;
...
#define REGISTERED_PERIPHERAL  0x0020

#ifdef QUAD_ENCODER
#include "inc_encoder.h"
INC_ENCODER encoderQ4;
#endif
...
#if defined(PROXISENSOR_VL53L0X) || defined(LSM6DSL_ACCGYR) || defined(TCS34725_COLORVIEW)
TwoWire WIRE1(PB11, PB10);  //SDA=PB11 & SCL=PB10
bool is_pin_protected(byte p){
  return ((p==38)||(p==39)||(p==33)||(p==34)||(p=D12));
}
#endif
...
#define TCS34725_MAXRANGE     ((uint16_t)20480)
#endif
...
void sysex_callback_extended(byte command, byte argc, byte *argv){
  switch(command) {
    ...
    case SYSEX_CMD_COLOR:
      ...
      switch(argv[0]){
        case 0x00 : // demande du range max du capteur
          uint8_t msg[3];
          msg[0] = 0x01;  // TCS reply
          msg[1] = TCS34725_MAXRANGE>>8;
          msg[2] = TCS34725_MAXRANGE&0x00FF;
          Firmata.sendSysex(0x02, sizeof(msg) , msg );
          break;
      }
    break;
  #endif    
  ...
  //      argc=decodeByteStream(argc,argv); // decode le stream
      ...
      switch(argv[0])     // traite la commande demandée
      {
        ...
        case 0x06 : // Bitmap
          ...
          break;
        case 0x07 : // setColor (linear LED, not screen)
                    // 1 : nb layer
                    // 2 : led nb
                    // 3 : alpha
                    // 4->6 : r,g,b
          argc=decodeByteStream(argc,argv); // decode le stream
          WS2812Screen.SetPixelAt(argv[1],argv[2],argv[3],argv[4],argv[5],argv[6]);
          break;
        case 0x20 : // demande le nombre de LED.
                    // la réponse se fait au travers de la sysex 0x02 sur un octet
          uint8_t msg[2];
          msg[0] = 0x00;
          msg[1] = WS2812B_NBLED;
          Firmata.sendSysex(0x02, sizeof(msg) , msg );
...
void execute_extended(void) {
  ...
  #ifdef QUAD_ENCODER
  list.value32[0] = (uint32_t)encoderQ4.GetTimerCounter();
  #endif
  ...
  #if defined(TCS34725_COLORVIEW) || defined(LSM6DSL_ACCGYR) || defined(PROXISENSOR_VL6180X) || defined(PROXISENSOR_VL53L0X) || defined(QUAD_ENCODER)
